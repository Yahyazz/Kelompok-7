...
class Parser:
    ...
    def statement(self):
        ...
        if self.current_tok.matches(token.T_KEYWORD, 'kembalikan'):
            res.register_advancement()
            self.advance()
        ...
        if self.current_tok.matches(token.T_KEYWORD, 'lanjut'):
            res.register_advancement()
            self.advance()
        ...
        if res.error:
            return res.failure(errors.InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'break', 'lanjut', 'kembalikan', 'item', 'jika', 'untuk', 'ketika', 'fungsi', int, float, identifier, '+', '-', '(', ')' '[' or 'bukan'"
            ))
        ...
        if self.current_tok.type == token.T_LPAREN:
            ...
            if self.current_tok.type == token.T_RPAREN:
                ...
                if res.error:
                    return res.failure(errors.InvalidSyntaxError(
                        self.current_tok.pos_start, self.current_tok.pos_end,
                        "Expected ')', 'item', 'jika', 'untuk', 'ketika', 'fungsi', int, float, identifier, '+', '-', '(', '[' or 'bukan'"
                    ))
    ...
    def atom(self):
        ...
        elif tok.matches(token.T_KEYWORD, 'jika'):
            if_expr = res.register(self.if_expr())
            if res.error:
                return res
            return res.success(if_expr)

        elif tok.matches(token.T_KEYWORD, 'untuk'):
            for_expr = res.register(self.for_expr())
            if res.error:
                return res
            return res.success(for_expr)

        elif tok.matches(token.T_KEYWORD, 'ketika'):
            while_expr = res.register(self.while_expr())
            if res.error:
                return res
            return res.success(while_expr)

        elif tok.matches(token.T_KEYWORD, 'fungsi'):
        ...
        return res.failure(errors.InvalidSyntaxError(
            ...
            "Expected int or float, identifier, '+', '-' or '(', , '[', 'jika', 'untuk', 'ketika' or 'fungsi'"
        ))
    ...
    def comp_expr(self):
        ...
        if self.current_tok.matches(token.T_KEYWORD, 'bukan'):
        ...
        node = res.register(self.bin_op(self.arithm_expr, (token.T_EE,
                            token.T_NE, token.T_LT, token.T_GT, token.T_LTE, token.T_GTE)))

        if res.error:
            return res.failure(errors.InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected int or float, identifier, '+', '-', '(', '[' or 'bukan'"
            ))
    ...
    def expr(self):
        ...
        if self.current_tok.matches(token.T_KEYWORD, 'item'):
            res.register_advancement()
            self.advance()
        ...
        node = res.register(self.bin_op(
            self.comp_expr, ((token.T_KEYWORD, 'dan'), (token.T_KEYWORD, 'atau'))))

        if res.error:
            return res.failure(errors.InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected int or float, identifier, 'item', 'jika', 'untuk', 'ketika', 'fungsi', '+', '-', '(' or '['"
            ))
    ...
    def if_expr_cases(self, case_keyword):
        ...
        if self.current_tok.type == token.T_NEWLINE:
            ...
            if self.current_tok.matches(token.T_KEYWORD, 'akhir'):
                res.register_advancement()
                self.advance()
            else:
    ...
    def if_expr_b(self):
        return self.if_expr_cases('dll')

    def if_expr_c(self):
        res = ParseResult()
        else_case = None

        if self.current_tok.matches(token.T_KEYWORD, 'lainnya'):
            res.register_advancement()
            self.advance()
    ...
    def if_expr_b_or_c(self):
        ...
        if self.current_tok.matches(token.T_KEYWORD, 'dll'):
    ...
    def for_expr(self):
        ...
        if not self.current_tok.matches(token.T_KEYWORD, 'untuk'):
            return res.failure(errors.InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'untuk'"
            ))
        ...
        if self.current_tok.type == token.T_EQ:
            ...
            if not self.current_tok.matches(token.T_KEYWORD, 'sampai'):
                return res.failure(errors.InvalidSyntaxError(
                    self.current_tok.pos_start, self.current_tok.pos_end,
                    f"Expected 'sampai'"
                ))
            ...
            if self.current_tok.matches(token.T_KEYWORD, 'langkah'):
                res.register_advancement()
                self.advance()
            ...
            if self.current_tok.type == token.T_NEWLINE:
                ...
                if not self.current_tok.matches(token.T_KEYWORD, 'akhir'):
                    return res.failure(errors.InvalidSyntaxError(
                        self.current_tok.pos_start, self.current_tok.pos_end,
                        f"Expected 'akhir'"
                    ))
        ...
        elif self.current_tok.matches(token.T_KEYWORD, 'dalam'):
            res.register_advancement()
            self.advance()
    ...
    def while_expr(self):
        ...
        if not self.current_tok.matches(token.T_KEYWORD, 'ketika'):
            return res.failure(errors.InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'ketika'"
            ))
    ...
    def func_def(self):
        ...
        if not self.current_tok.matches(token.T_KEYWORD, 'fungsi'):
            return res.failure(errors.InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'fungsi'"
            ))
