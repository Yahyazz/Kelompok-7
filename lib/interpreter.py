...
class BaseFunction(Value):
    ...
    def check_and_populate_args(self, arg_names, optional_arg_names, optional_arg_values, args, exec_ctx):
        ...
        if res.should_return():
            return res
        self.populate_args(arg_names, args, optional_arg_names,
                           optional_arg_values, exec_ctx)
...
class Function(BaseFunction):
    ...
    def execute(self, args, optional_arg_names, optional_arg_values):
        ...
        res.register(self.check_and_populate_args(
            self.arg_names, self.optional_arg_names, self.optional_arg_values, args, exec_ctx))
        if res.should_return():
            return res
    ...
    def copy(self):
        copy = Function(self.name, self.body_node, self.arg_names, self.optional_arg_names,
                        self.optional_arg_values, self.should_auto_return, self.module)
        copy.set_context(self.context)
        copy.set_pos(self.pos_start, self.pos_end)
        return copy
...
class BuiltInFunction(BaseFunction):
    ...
    ##############################################
    def execute_cetak(self, exec_ctx):
        print(str(exec_ctx.symbol_table.get('value')))
        return RTResult().success(Number.null)
    execute_cetak.arg_names = ['value']

    def execute_masukan(self, exec_ctx):
        text = input()
        return RTResult().success(String(text))
    execute_masukan.arg_names = []
...
# Add ends
    ...
    def execute_imports(self, exec_ctx):
        ...
        self.execute_jalankan(exec_ctx, importFile=True,
    ...
    def execute_jalankan(self, exec_ctx, importFile=False, module_name=""):
    ...
    def execute_jangkauan(self, exec_ctx):
    ...
    execute_jangkauan.arg_names = ['begin', 'end']
...
BuiltInFunction.cetak = BuiltInFunction("cetak")
BuiltInFunction.masukan = BuiltInFunction("masukan")
...
BuiltInFunction.jangkauan = BuiltInFunction("jangkauan")
...
class Interpreter:
    ...
    def visit_DictNode(self, node, context):
        ...
        for key in node.element_nodes:
            dict[res.register(self.visit(key, context))] = res.register(
                self.visit(node.element_nodes[key], context))
            if res.should_return():
                return res

        return res.success(
            Dict(dict).set_context(context).set_pos(
                node.pos_start, node.pos_end)
        )
    ...
    def visit_ForEachNode(self, node, context):
        ...
        for i in arr.elements:
            ...
            if res.should_return() and not res.loop_should_break and not res.loop_should_continue:
                return res
    ...
    def visit_FunctionDefNode(self, node, context):
        ...
        optional_arg_names = [
            optional_arg.value for optional_arg in node.optional_arg_names]
        optional_arg_values = []
        ...
        func_value = Function(func_name, body_node, arg_names, optional_arg_names, optional_arg_values,
                              node.should_auto_return, module).set_context(context).set_pos(node.pos_start, node.pos_end)
    ...
    def visit_CallNode(self, node, context):
        ...
        for i in range(len(node.optional_arg_names)):
            ...
            optional_arg_values.append(res.register(
                self.visit(node.optional_arg_values[i], context)))

        return_value = res.register(value_to_call.execute(
            args, optional_arg_names, optional_arg_values))
        if res.should_return():
            return res
...
def reset_global_symbol_table():
    ...
    global_symbol_table.set("cetak", BuiltInFunction.cetak)
    global_symbol_table.set("masukan", BuiltInFunction.masukan)
    ...
    global_symbol_table.set("jangkauan", BuiltInFunction.jangkauan)
